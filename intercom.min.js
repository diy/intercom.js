...
b=b&&b.key;if(!b||b===h){for(var b=JSON.parse(f.getItem(h)||"[]"),a=0;a<b.length;a++)if(b[a].origin!==this.origin&&!(b[a].timestamp<this.lastMessage)){if(b[a].id){if(this.receivedIDs.hasOwnProperty(b[a].id))continue;this.receivedIDs[b[a].id]=!0}this.trigger(b[a].name,b[a].payload)}this.lastMessage=c}this._cleanup()};e.prototype._emit=function(b,c,a){if((a="string"===typeof a||"number"===typeof a?String(a):null)&&a.length){if(this.receivedIDs.hasOwnProperty(a))return;this.receivedIDs[a]=!0}var a={id:a,
name:b,origin:this.origin,timestamp:(new Date).getTime(),payload:c},d=f.getItem(h)||"[]",d=[d.substring(0,d.length-1),"[]"===d?"":",",JSON.stringify(a),"]"].join("");f.setItem(h,d);this.trigger(b,c)};e.prototype.bind=function(b,c){for(var a=0;a<e.bindings.length;a++){var d=e.bindings[a].factory(b,c||null,this);d&&this.bindings.push(d)}};e.prototype.emit=function(b,c){this._emit.apply(this,arguments);this.trigger("intercom:emit",b,c)};e.prototype.once=function(b,c){if(e.supported){var a=JSON.parse(f.getItem(j)||
